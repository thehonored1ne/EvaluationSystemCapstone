...
new #[Layout('components.layouts.app')] class extends Component
{
    ...
    {
        $staffProcessing = $this->departmentStaff->contains(function ($staff) {
            return $staff->user && $this->getEvaluationStatus($staff->user->id, 'department_head') === 'processing';
        });
        ...
        return $this->deans->contains(function ($dean) {
            return $dean->user && $this->getEvaluationStatus($dean->user->id, 'upward_employee') === 'processing';
        });
    }
                ...
                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-x-2 gap-y-1 mt-1.5 text-xs text-zinc-500 dark:text-zinc-400">
                    ...
                    @else
                        <span class="font-medium text-amber-600 dark:text-amber-400">
                            Administrative department not assigned
                        </span>
                    @endif
    ...
    @elseif($this->employee?->department_id)
        ...
        <div class="flex items-center border-b border-zinc-200 dark:border-zinc-800 w-full overflow-x-auto">
            ...
            <button 
                ...
                class="flex-1 sm:flex-none pb-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold transition-all cursor-pointer text-center border-b-[3px] {{ $tab === 'staff' ? 'border-[#9b0000] dark:border-[#a82e2e] text-[#9b0000] dark:text-[#e07a7a]' : 'border-transparent text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200 hover:border-zinc-300 dark:hover:border-zinc-700' }}">
                Staff
            </button>
            ...
            <button 
                ...
                class="flex-1 sm:flex-none pb-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold transition-all cursor-pointer text-center border-b-[3px] {{ $tab === 'dean' ? 'border-[#9b0000] dark:border-[#a82e2e] text-[#9b0000] dark:text-[#e07a7a]' : 'border-transparent text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200 hover:border-zinc-300 dark:hover:border-zinc-700' }}">
                Dean
            </button>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6">
            <!-- 1. Self Evaluation -->
            @if($tab === 'self')
                <flux:card wire:key="department-head-tab-content-self" class="p-3.5 sm:p-5">
            ...
            <!-- 2. Staff Evaluation -->
            @if($tab === 'staff')
                <flux:card wire:key="department-head-tab-content-staff" class="p-3.5 sm:p-5">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                        <div class="text-center sm:text-left">
            ...
            <!-- 3. Dean Evaluation -->
            @if($tab === 'dean')
                <flux:card wire:key="department-head-tab-content-dean" class="p-3.5 sm:p-5">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                        <div class="text-center sm:text-left">
