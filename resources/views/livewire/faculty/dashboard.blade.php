...
new #[Layout('components.layouts.app')] class extends Component
{
    ...
    public function getHasProcessingProperty(): bool
    {
        ...
        $peersProcessing = $this->peers->contains(function ($peer) {
        ...
        return $this->programHeads->contains(function ($head) {
...
<div class="flex flex-col gap-6 sm:gap-8 w-full max-w-6xl mx-auto px-0 sm:px-3 md:px-4 py-3 sm:py-6"
    ...
    @elseif($this->employee?->department_id)
        ...
        <div class="grid grid-cols-1 gap-6">
            <!-- 1. Self Evaluation -->
            @if($tab === 'self')
                <flux:card wire:key="faculty-tab-content-self" class="p-3.5 sm:p-5">
            ...
            <!-- 2. Peer Evaluation -->
            @if($tab === 'peer')
                <flux:card wire:key="faculty-tab-content-peer" class="p-3.5 sm:p-5">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                        <div class="text-center sm:text-left">
                                ...
                                @if($peer->user)
                                    ...
                                    <div class="bg-white dark:bg-zinc-900 p-3 sm:p-3.5 rounded-xl border border-zinc-200 dark:border-zinc-800 flex flex-col gap-3 shadow-2xs">
                                        <div class="flex items-start justify-between gap-3">
                                            ...
                                            <div class="shrink-0">
                                                ...
                                                @elseif($status === 'exempted')
                                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700">
                                                        <flux:icon icon="no-symbol" class="size-3.5 text-zinc-500" />
                                                        Skipped (No Basis)
                                                    </span>
                                                @elseif($status === 'processing')
                                        ...
                                        <div class="pt-2 border-t border-zinc-100 dark:border-zinc-800 flex justify-end">
                                            ...
                                            @elseif($status === 'exempted')
                                                <span class="text-xs text-zinc-400 font-semibold py-1">Exempted</span>
                                            @elseif($status === 'processing')
                                        ...
                                        @if($peer->user)
                                            ...
                                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/20 transition-colors">
                                                ...
                                                <td class="px-6 py-4 text-right">
                                                    ...
                                                    @elseif($status === 'exempted')
                                                        <span class="text-xs text-zinc-400 font-semibold">Exempted</span>
                                                    @elseif($status === 'processing')
            ...
            @if($tab === 'supervisor')
                <flux:card wire:key="faculty-tab-content-supervisor" class="p-3.5 sm:p-5">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                        <div class="text-center sm:text-left">
