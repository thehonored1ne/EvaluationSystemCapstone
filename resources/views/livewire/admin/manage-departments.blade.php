...
new #[Layout('components.layouts.app')] class extends Component
{
    ...
    {
        return view('livewire.placeholders.manage-departments-skeleton');
    }
...
<div class="w-full flex flex-col gap-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full text-left mb-2">
        <div class="flex flex-col items-start text-left">
            <flux:heading size="xl" level="1" class="text-left !text-lg sm:!text-xl font-bold tracking-tight">Manage Departments</flux:heading>
        </div>
        <flux:button variant="primary" wire:click="prepareCreate" icon="plus" class="w-full sm:w-auto justify-center">Add Department</flux:button>
    </div>
    ...
    <div x-data="{ showMobileFilters: false }" class="flex flex-col gap-2.5 sm:gap-3">
        <div class="flex items-center gap-2 w-full">
            ...
            @php
                $activeFiltersCount = ($typeFilter ? 1 : 0) + ($headFilter ? 1 : 0) + ($sortBy && $sortBy !== 'name_asc' ? 1 : 0);
            @endphp
            ...
            @if($search || $typeFilter || $headFilter || ($sortBy && $sortBy !== 'name_asc'))
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-zinc-900 p-6 rounded-xl shadow-xl w-full max-w-lg border border-zinc-200 dark:border-zinc-800 overflow-y-auto max-h-[90vh] space-y-4">
            ...
            <form wire:submit="saveDepartment" class="flex flex-col gap-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-zinc-900 dark:text-zinc-100 mb-1">Department Type</label>
                    <select wire:model.live="type" class="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm focus:border-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
