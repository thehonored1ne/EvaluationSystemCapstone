...
<div class="w-full flex flex-col gap-6">
    <div class="flex justify-between items-center">
        <flux:heading size="xl" level="1">Manage Staff</flux:heading>
        <flux:button variant="primary" wire:click="prepareCreate" icon="plus">Create Staff</flux:button>
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="bg-white dark:bg-zinc-900 p-6 rounded-lg shadow-xl w-full max-w-lg border border-zinc-200 dark:border-zinc-700 overflow-y-auto max-h-[90vh]">
            <flux:heading size="lg" class="mb-4">{{ $editingUser ? 'Edit Staff' : 'Create Staff' }}</flux:heading>
            ...
            <form wire:submit="{{ $editingUser ? 'updateUser' : 'createUser' }}" class="flex flex-col gap-4">
                ...
                <flux:input wire:model="employee_number" label="Employee Number" type="text" placeholder="e.g. STF-001" required />
    ...
    >
        Are you sure you want to delete this staff account? This action cannot be undone and will remove all login access.
        ...
        @if(\App\Models\Evaluation::where('evaluator_id', $deletingUser->id)->orWhere('evaluatee_id', $deletingUser->id)->exists())
            <x-slot:warning>
                Deleting this staff member will permanently delete all evaluations they submitted or received.
            </x-slot:warning>
        @endif
    </x-confirmation-modal>
    @endif
