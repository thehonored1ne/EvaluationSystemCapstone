...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function resetPeriodFilters(): void
    {
        $this->searchYear = '';
        $this->semesterFilter = '';
        $this->resetPage();
    ...
    public function getSemestersPaginatedProperty()
    {
        ...
        if (trim($this->searchYear) !== '') {
            $term = '%' . trim($this->searchYear) . '%';
            $query->where('academic_years.name', 'like', $term);
        }

        if ($this->semesterFilter !== '') {
            $query->where('semesters.name', $this->semesterFilter);
        }
    ...
    {
        Semester::query()->update(['is_active' => false, 'is_evaluation_open' => false]);
...
<div class="w-full flex flex-col gap-6 text-left">
    ...
    <!-- SECTION 1: Academic Years & Semesters Paginated Table -->
    <div id="academic-periods-section" class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 sm:p-6 shadow-xs flex flex-col gap-4 sm:gap-6 w-full">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 flex-1">
                <div class="w-full sm:w-64">
                    <flux:input 
                        type="search" 
                        wire:model.live.debounce.300ms="searchYear" 
                ...
                <div class="w-full sm:w-44">
                    <flux:select wire:model.live="semesterFilter" size="sm">
                ...
                @if($searchYear || $semesterFilter)
                    <flux:button wire:click="resetPeriodFilters" size="sm" variant="subtle" icon="x-mark">
                        Clear
                    </flux:button>
                @endif
            </div>
        ...
        <!-- Fixed-Height Table Container with Sticky Header -->
        <div class="w-full overflow-x-auto rounded-xl border border-gray-200 dark:border-zinc-700 shadow-xs max-h-[380px] overflow-y-auto overscroll-contain">
            <table class="w-full min-w-[750px] divide-y divide-gray-200 dark:divide-zinc-700 text-sm text-left">
                <thead class="bg-gray-50 dark:bg-zinc-800 text-xs font-semibold text-gray-700 dark:text-zinc-300 uppercase tracking-wider sticky top-0 z-10">
                    <tr>
                        <th class="w-[28%] min-w-[180px] px-4 py-3.5 whitespace-nowrap bg-gray-50 dark:bg-zinc-800">Academic Year</th>
                        <th class="w-[26%] min-w-[160px] px-4 py-3.5 whitespace-nowrap bg-gray-50 dark:bg-zinc-800">Semester Name</th>
                        <th class="w-[18%] min-w-[130px] px-4 py-3.5 text-center whitespace-nowrap bg-gray-50 dark:bg-zinc-800">Evaluation Window</th>
                        <th class="w-[14%] min-w-[100px] px-4 py-3.5 text-center whitespace-nowrap bg-gray-50 dark:bg-zinc-800">Status</th>
                        <th class="w-[14%] min-w-[100px] px-4 py-3.5 text-right whitespace-nowrap bg-gray-50 dark:bg-zinc-800">Actions</th>
                    </tr>
                </thead>
