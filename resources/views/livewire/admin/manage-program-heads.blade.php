...
new #[Layout('components.layouts.app')] #[Lazy] class extends Component {
    ...
    public function deleteUser()
    {
        ...
        \Flux::toast(
            heading: 'Program Head Deleted',
            text: 'The program head account has been successfully deleted.',
            variant: 'success'
        );
    }
}; ?>
...
<div class="w-full flex flex-col gap-6">
    ...
    <x-confirmation-modal 
        title="Delete Program Head Account" 
        on-confirm="deleteUser" 
        on-cancel="$set('showDeleteModal', false)"
    >
        Are you sure you want to delete this program head account? This action cannot be undone and will remove all login access.
        ...
        @if(\App\Models\Evaluation::where('evaluator_id', $deletingUser->id)->orWhere('evaluatee_id', $deletingUser->id)->exists())
            <x-slot:warning>
                Deleting this program head will permanently delete all evaluations they submitted or received.
            </x-slot:warning>
        @elseif($deletingUser->employee?->managedProgram()->exists())
            <x-slot:warning>
                This program head is currently managing the program: {{ $deletingUser->employee->managedProgram->name }}. Deleting them will leave the program without a program head.
            </x-slot:warning>
        @endif
    </x-confirmation-modal>
    @endif
