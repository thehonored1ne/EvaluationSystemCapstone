...
<div class="w-full flex flex-col gap-6">
    ...
    <div class="flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-center sm:justify-between mb-2">
        <div>
            <h1 class="text-lg sm:text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Manage Employees</h1>
        </div>
        ...
        <div class="flex items-center gap-2 w-full sm:hidden">
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    <flux:menu.item icon="arrow-down-tray" wire:click="exportEmployees">
                        Export CSV
                    </flux:menu.item>
    ...
    <div x-data="{ showMobileFilters: false }" class="flex flex-col gap-2.5 sm:gap-3">
        <div class="flex items-center gap-2 w-full">
            ...
            @php
                $activeFiltersCount = ($selectedRole ? 1 : 0) + ($selectedEmploymentType ? 1 : 0) + ($selectedStatus ? 1 : 0) + ($selectedDepartmentId ? 1 : 0);
            @endphp
        ...
        <div 
            ...
            x-cloak
            class="flex flex-col sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-3 p-4 sm:p-0 bg-zinc-50 sm:bg-transparent dark:bg-zinc-800/60 sm:dark:bg-transparent rounded-xl border border-zinc-200/80 sm:border-0 dark:border-zinc-700/60 transition-all duration-200 sm:!grid"
            :class="{ 'hidden': !showMobileFilters }"
        >
            <!-- Role Filter Dropdown -->
            <div class="w-full">
                <flux:select wire:model.live="selectedRole" placeholder="All Roles" class="w-full">
            ...
            <div class="w-full">
                <flux:select wire:model.live="selectedEmploymentType" placeholder="All Types" class="w-full">
            ...
            <div class="w-full">
                <flux:select wire:model.live="selectedStatus" placeholder="All Status" class="w-full">
            ...
            @if($search || $selectedRole || $selectedEmploymentType || $selectedStatus || $selectedDepartmentId || $sortDirection !== 'asc')
                <div class="flex sm:hidden items-center justify-between pt-1 border-t border-zinc-200/60 dark:border-zinc-700/60 sm:col-span-2 lg:col-span-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3 bg-zinc-900 text-white dark:bg-zinc-800 dark:border dark:border-zinc-700 px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-xl shadow-lg animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div class="grid grid-cols-3 gap-2 w-full sm:hidden pt-0.5">
                ...
                <flux:dropdown align="start" class="w-full">
                    ...
                    <flux:menu>
                        <flux:menu.item icon="identification" wire:click="$set('showBulkStatusModal', true)">
                            Change Status
                        </flux:menu.item>
                        <flux:menu.item icon="building-office-2" wire:click="$set('showBulkDeptModal', true)">
                            Assign Dept
                        </flux:menu.item>
                        <flux:menu.item icon="briefcase" wire:click="$set('showBulkEmploymentModal', true)">
                            Set Employment
                        </flux:menu.item>
                    </flux:menu>
                </flux:dropdown>
